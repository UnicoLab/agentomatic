...
# Keep the exact framework pin generated by `agentomatic new`.
agentomatic[all]==<released-version>
# This project's agents talk to a local OpenAI-compatible model server,
# so it needs that client library.
langchain-openai>=0.3
```

Replace `<released-version>` only when you intentionally upgrade and re-run
the repository verification gates. The scaffold writes the concrete version
that created the project, so generated projects do not contain this placeholder.
