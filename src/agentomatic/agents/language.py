"""Lightweight language detection and prompt output directives.

Agents typically *reason* in English (prompts stay English) but render
user-facing output in the caller's language. Pass an explicit ``language``
parameter, or detect from input text and append :func:`output_directive` to
the prompt.
"""
...
def resolve_language(explicit: str | None, *texts: str, default: str = "en") -> str:
...
def output_directive(code: str) -> str:
    ...
    Avoid phrasing that nudges models into a visible chain-of-thought
    preamble (that burns ``max_tokens`` and yields empty JSON after
    thinking-strip).
    """
    return (
...
language_directive = output_directive
