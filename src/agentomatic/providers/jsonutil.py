...
Small models frequently wrap JSON in prose or code fences, emit trailing
commas, or truncate. These helpers extract the first JSON value and apply
conservative repairs before falling back to ``None`` so callers can trigger
a rule-based fallback path.

Thinking / reasoning preambles are stripped via
:func:`~agentomatic.providers.message_utils.strip_thinking_for_json`.
"""
...
def extract_json(text: str, *, expect: str = "object") -> Any | None:
    ...
    for attempt in (body, repair_json(body)):
        try:
    ...
    for open_ch, close_ch in pairs:
        ...
        while True:
            ...
            if span:
                ...
                break
        for span in spans:
            for attempt in (span, repair_json(span)):
                try:
                    return json.loads(attempt)
                except (json.JSONDecodeError, ValueError):
                    continue
...
def loads_repaired(text: str, *, expect: str = "object") -> Any | None:
    """Alias for :func:`extract_json` (parse with repair, or ``None``)."""
