...
Pipelines write a full artifact *bundle* to a candidate version directory,
validate it against an eval gate, then atomically flip the ``current``
pointer. Rollback re-points ``current`` at any prior version. Each version
records metadata, model cards and eval scores.

Bundle layout is caller-defined. A typical version directory looks like::

    <root>/<version>/
        model.pkl            # plugin weights
        metadata.jsonl       # optional aligned metadata
...
def default_artifact_root() -> Path:
    """Resolve the default artifact root from env / platform settings.

    Resolution order:
      1. ``AGENTOMATIC_ARTIFACT_ROOT``
      2. ``PlatformSettings.artifact_root`` when settings are available
      3. ``.local/artifacts``
    """
    env = (os.getenv("AGENTOMATIC_ARTIFACT_ROOT") or "").strip()
    if env:
        return Path(env)
    try:
...
class ArtifactRegistry:
    ...
    def _save_registry(self, data: dict[str, Any]) -> None:
        """Atomically write the registry file (tmp + fsync + replace)."""
        ...
        try:
            ...
            # After a successful replace the tmp path is gone; ignore races.
            tmp_path.unlink(missing_ok=True)
