"""Live prompt-optimization smoke against OpenAI cloud.

Skipped unless a real ``OPENAI_API_KEY`` (``sk-…``) is set. A local
``OPENAI_BASE_URL`` pointing at oMLX/vLLM is ignored for cloud model ids
so this suite can run alongside local-dev env vars.

Run with::

    export OPENAI_API_KEY=sk-…
    export AGENTOMATIC_LIVE_OPENAI_MODEL=gpt-4o-mini   # optional
    uv run pytest tests/test_live_openai_optimize.py -q --override-ini='addopts='
"""
...
_RAW_MODEL = os.getenv("AGENTOMATIC_LIVE_OPENAI_MODEL", "gpt-4o-mini")
MODEL = _RAW_MODEL if _RAW_MODEL.startswith("openai/") else f"openai/{_RAW_MODEL}"
_BASELINE_PROMPT = "You are a vague assistant."


def _openai_cloud_ready() -> bool:
...
pytestmark = [
    pytest.mark.live,
    pytest.mark.skipif(
        not _openai_cloud_ready(),
        reason="OPENAI_API_KEY (sk-…) not set — skipping live OpenAI tests",
    ),
]
...
def _agent_dataset() -> AgentDataset:
    ...
    return AgentDataset(name="live_openai", examples=examples)


@pytest.fixture(autouse=True)
def _clear_local_openai_hijack(monkeypatch: pytest.MonkeyPatch) -> None:
    """Ensure class-level local configure from other tests cannot leak in."""
    LLMCaller.configure()
    # Keep OPENAI_API_KEY; local OPENAI_BASE_URL is ignored for gpt-* by design.


def test_openai_model_is_llm_multipass() -> None:
    assert parse_model_spec(MODEL)[0] == "openai"
    assert looks_like_slm(MODEL) is False
    assert resolve_rewrite_passes(MODEL) == 2


@pytest.mark.asyncio
async def test_openai_caller_clean_text_and_json() -> None:
...
@pytest.mark.asyncio
async def test_openai_multipass_rewrite_improves() -> None:
    agent = _EchoAgent()
    train, val = _opt_splits()
    fitter = PromptFitter(
        agent="live_openai_echo",
        task_model=MODEL,
        rewrite_model=MODEL,
        optimizer="rewrite",
        max_trials=4,
...
@pytest.mark.asyncio
async def test_openai_llm_judge() -> None:
...
def test_openai_prompt_fitter_bridge_rewrite() -> None:
    agent = _EchoAgent()
    ds = _agent_dataset()
    metrics = [
